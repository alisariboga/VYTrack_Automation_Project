...
import static com.vytrack.utilities.BasePage.logger;
...
public class ExcelUtil {
    ...
    public ExcelUtil(String sheetName) {
        //We are getting path to Excel file from properties file
        this.path = ConfigurationReader.getProperty("test_data");
        try {
            // Open the Excel file
            FileInputStream ExcelFile = new FileInputStream(path);
            // Access the required test data sheet
            //Then, worksheet, contains table test data sheet
            workBook = WorkbookFactory.create(ExcelFile);
            workSheet = workBook.getSheet(sheetName);
